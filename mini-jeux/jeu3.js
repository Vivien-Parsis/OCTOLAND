...
function preload()
{
  ...
  this.load.image('josiane', '../assets/image/jeu3/josiane.png');
  for(let i = 0; i<keyImageOcto.length; i++)
  {
    this.load.image(keyImageOcto[i],`../assets/image/jeu3/${keyImageOcto[i]}.png`);   
  }
...
function create()
{
  ...
  this.waitingtext = this.add.text(config.width/3,config.height/2,'',
    {fontfamily:"arial",fill:'#eeee00',stroke:'#222222',strokeThickness:6});
...
function update()
{
  if(waiting)
    {
      ...
      this.scoreTest.setText("appuyer sur espace pour commencer");
      this.scoreTest.setPosition((config.width/2)-(this.scoreTest.displayWidth/2),config.height/2);
      for(let i = 0; i<this.octoList.length; i++)
      {this.octoList[i].setVisible(false);}
      if(this.spaces.isDown)
      {
        for(let i = 0; i<this.octoList.length; i++)
        {this.octoList[i].setVisible(true);}
        this.scoreTest.setPosition(0,0);
        waiting = false;
      }
      return;
    }
  ...
  if(timeCount == 0 && this.pause == false)
  {
    ...
    const text = "Score: "+this.score+'\nMeilleur score: '+ highScore+'\n\nPresser espace pour redémarrer !';
    this.scoreTest.setText(text);
  ...
  if(this.pause && this.spaces.isDown)
  {
  ...
  if(this.pause == false && (this.spaces.isDown || pointer.isDown
...
//genere un nombre entre [min, max[
